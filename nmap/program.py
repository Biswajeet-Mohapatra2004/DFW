...
def scan_ip_mac_and_port_status(network_range, port):
    try:
        # Run nmap command to scan IP, MAC, and port 80 status
        result = subprocess.run(
            ["sudo", "nmap", "-p", str(port), network_range],
        ...
        ip_regex = r"Nmap scan report for ([0-9+\.]+)"
        mac_regex = r"MAC Address: ([0-9A-Fa-f:]+)"
        port_status_regex = rf"{port}/tcp\s+(\w+)"
...
# Example usage
network = input("Enter a network ip/cidr: ")  # Replace with your network range
port=int(input("Enter a port: "))
devices = scan_ip_mac_and_port_status(network,port)
